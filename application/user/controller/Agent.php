		...
		if($id <> 0){
			if(!isPost()){
	            ...
	            if($data['status'] == '2'){
                	...
                	$levelOrderMoney = $level['money'] / ($level['rebate'] / 10) * ($levelRatio / 10);
	                if($level['auditing'] == '1'){
	                	// 由上级审核，验证款项
	                	if($levelOrderMoney > $this->user_data['money']){
	                		return $this->error('账户款项不足'.$levelOrderMoney.'元，请先充值');
	                		exit();
	                	}
	                }else if($level['auditing'] == '2'){

	                }else if($level['auditing'] == '3'){
	                ...
	                db('agent_profit')->insertGetId([
	                    ...
	                    'form_id'=>$info['uid'],
	                    'money'=>($level['money'] - $levelOrderMoney),
	                    'timestamp'=>date('Y-m-d H:i:s')
	                ]);
	                // 计算发放佣金推荐
	                $reward = new \lib\Reward;
	                $reward->push($info['uid']);
	                $reward->same($info['uid'],$level['money']);
	                // 业绩计算
	                if($info['parent_id'] > 0){
	                    ...
	                    if($info['level_id'] == $parent['level_id']){
	                        db('performance')->insertGetId([
	                            ...
	                            'form_id'=>$info['uid'],
	                            'type'=>'1',
	                            'money'=>$orderFirstMoney,
	                            'timestamp'=>date('Y-m-d H:i:s')
	                        ]);
	                        $reward->teamreward($parent['id'],$orderFirstMoney);
	                    }
	                }
	                // 扣除审核人账户款项
	                db('user')->where('id',$this->user_data['id'])->update(['money'=>($this->user_data['money'] - $levelOrderMoney)]);              
	                // 更新被审核人账户余额
	                ...
	                $state = db('user')->where('id',$info['uid'])->update(['status'=>'2']);

	                if($state){
	                    $authLink = '<a href="'.urlDiy('/user/index').'">前往代理后台</a>';
	                    $msgData = "亲，你的代理授权申请由上级审核通过啦！\n审核人：".getUser($info['parent_id'],'auth_name')."\n审核时间：".date('Y-m-d H:i:s')."\n\n申请人：".getUser($info['uid'],'auth_name')."\n微信号：".getUser($info['uid'],'wechat')."\n手机号：".getUser($info['uid'],'phone')."\n\n申请级别：".$info['level_name']."\n\n请耐心等待总部二次审核".$authLink;
